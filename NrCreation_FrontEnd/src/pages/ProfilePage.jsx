...
import {
  fetchParticularCustomerOrders,
  setSelectedOrderFilters,
} from "@/store/slices/ordersSlice";
import { formatDate } from "@/utils/formatString";
import Pagination from "@/components/ReusableComponents/Pagination";
import OrderStatusBar from "@/components/ReusableComponents/OrderstatusBar";
...
const shippingMethodOptions = ["", "Standard", "Express", "Pickup"];
...
const ProfilePage = () => {
  ...
  const { customerOrders, selectedOrderFilters, totalPages, loading } =
    useSelector((state) => state.orders);
  const [tempOrderFilters, setTempOrderFilters] =
    useState(selectedOrderFilters);

  const toggle = (id) => setOpenOrderId((prev) => (prev === id ? null : id));
  ...
  const handleOrderPageChange = (newPage) => {
    dispatch(
      setSelectedOrderFilters({ ...selectedOrderFilters, page: newPage })
    );
  };
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-wrap gap-3 items-end bg-gray-50 p-4 rounded-lg border mb-4">
                  <Input
                    ...
                    onChange={(e) =>
                      handleOrderFilterChange("search", e.target.value)
                    }
                    className="max-w-xs"
                  />
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleOrderDateChange("startDate", e.target.value)
                      }
                      className="w-36"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleOrderDateChange("endDate", e.target.value)
                      }
                      className="w-36"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleOrderFilterChange("orderStatus", e.target.value)
                      }
                  ...
                  {customerOrders.map((order) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Order Status Bar - Moved here for full-width display */}
